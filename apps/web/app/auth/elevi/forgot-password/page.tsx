...
export default function StudentForgotPasswordPage() {
  ...
  const handleRequestCode = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/auth/students/password-reset/request", {
        method: "POST",
        headers: {
  ...
  const handleConfirmReset = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/auth/students/password-reset/confirm", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        <div className="relative mx-auto grid w-full max-w-7xl gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
          <section className="relative overflow-hidden rounded-[2.4rem] border border-white/10 bg-[#070b14]/95 p-6 shadow-[0_30px_120px_rgba(0,0,0,0.42)] backdrop-blur sm:p-8 lg:p-10">
            <div className="relative z-10 max-w-2xl">
              ...
              <p className="mt-6 max-w-xl text-base leading-8 text-gray-300 sm:text-lg">
                Trimiți cod pe email, confirmi codul și setezi parola nouă într-un flux securizat, fără linkuri nesigure.
              </p>
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                <div className="rounded-2xl border border-white/10 bg-[#050814]/95 p-4">
                  ...
                  <div className="mt-2 text-base font-semibold text-white">Introduci email</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-[#050814]/95 p-4">
                  <div className="text-xs uppercase tracking-[0.16em] text-slate-400">2</div>
                  <div className="mt-2 text-base font-semibold text-white">Primești codul</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-[#050814]/95 p-4">
                  <div className="text-xs uppercase tracking-[0.16em] text-slate-400">3</div>
                  <div className="mt-2 text-base font-semibold text-white">Setezi parola nouă</div>
                </div>
              </div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link href="/auth/elevi/signup" className="inline-flex items-center justify-center rounded-full border border-white/15 bg-white/5 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/10">
                  Creează cont nou
                </Link>
              </div>
            </div>
          ...
          <section className="relative overflow-hidden rounded-[2.4rem] border border-white/10 bg-[#070b14]/95 p-6 shadow-[0_30px_120px_rgba(0,0,0,0.5)] backdrop-blur sm:p-8 lg:p-10">
            ...
            <div className="mb-6 flex items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-cyan-300/80">Resetare parolă</p>
                <h2 className="mt-2 text-2xl font-bold text-white">
                  {step === "request" ? "Cere codul" : "Confirmă codul"}
                </h2>
              </div>
            </div>
              ...
              {step === "confirm" && (
                <>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <label className="block space-y-2">
                      <span className="text-sm font-medium text-gray-300">Confirmă parola nouă</span>
                      <input
                        type="password"
