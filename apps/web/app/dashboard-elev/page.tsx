  ...
  if (error) {
    return (
      <section className="rounded-3xl border border-white/10 bg-[#030712] p-6 text-white shadow-[0_8px_24px_rgba(15,23,42,0.03)]">
        ...
        <p className="mt-2 text-sm text-gray-300">Nu am putut încărca datele reale ale contului.</p>
    ...
    <section className="space-y-4 text-white">
      <div className="flex flex-wrap items-center justify-between gap-3 px-1 py-1">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-300">Date reale din contul tău, cursuri, progres și activitate.</p>
        </div>
        ...
        <div className="flex w-full flex-wrap gap-2 pt-1">
          {[
            ...
            <span
              key={chip}
              className={`rounded-full border px-3 py-1 text-xs font-semibold ${index === 0 ? "border-[#111827] bg-[#111827] text-white" : "border-white/10 bg-white/5 text-gray-300"}`}
            >
              {chip}
            </span>
          ))}
      ...
      <div className="grid gap-3 lg:grid-cols-4">
        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)]">
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-base font-semibold text-white">{profile?.fullName ?? "Elev"}</p>
              <p className="truncate text-sm text-gray-300">{profile?.email ?? "-"}</p>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
            <div className="rounded-2xl border border-white/10 bg-white/5 px-2.5 py-2 text-gray-200">
        ...
        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)]">
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-gray-300">Cursuri enrolled</p>
          <div className="mt-2 text-[30px] font-bold leading-none">{summary?.enrolledCourses ?? 0}</div>
          <p className="mt-1 text-xs text-gray-300">Cursuri reale asociate contului tău.</p>
          <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
            <div className="rounded-2xl border border-white/10 bg-white/5 px-2.5 py-2">Active: {summary?.activeCourses ?? 0}</div>
            <div className="rounded-2xl border border-white/10 bg-white/5 px-2.5 py-2">Finalizate: {summary?.completedCourses ?? 0}</div>
            <div className="rounded-2xl border border-white/10 bg-white/5 px-2.5 py-2">Lecții: {summary?.completedLessons ?? 0}/{summary?.totalLessons ?? 0}</div>
          </div>
        </article>

        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)]">
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-gray-300">XP total</p>
          <div className="mt-2 text-[30px] font-bold leading-none">{summary?.totalXp ?? 0}</div>
          <p className="mt-1 text-xs text-gray-300">Calculat din `xp_ledger`.</p>
          <div className="mt-4 h-2 rounded-full bg-[#e2e8f0]">
            <div className="h-full rounded-full bg-cyan-400" style={{ width: `${Math.min(100, summary?.profileCompletion ?? 0)}%` }} />
          </div>
          <div className="mt-2 text-xs text-gray-300">Profil completat: {summary?.profileCompletion ?? 0}%</div>
        </article>

        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)]">
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-gray-300">Comunitate</p>
          <div className="mt-2 text-[30px] font-bold leading-none">
            {data?.community.friendsAvailable ? (data.community.friendsCount ?? 0) : "--"}
          </div>
          <p className="mt-1 text-xs text-gray-300">{data?.community.note ?? "Zona socială este pregătită."}</p>
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            <div className="rounded-2xl border border-white/10 bg-white/5 px-2.5 py-2">Notificări: {data?.community.notificationsCount ?? 0}</div>
            <div className="rounded-2xl border border-white/10 bg-white/5 px-2.5 py-2">Support: {data?.community.supportTicketsCount ?? 0}</div>
          </div>
        </article>
      </div>
      ...
      <div className="grid gap-3 xl:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)] sm:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-semibold text-white">Progres pe cursuri</h3>
              <p className="mt-1 text-xs text-gray-300">Date reale din `enrollments`, `progress`, `lessons` și `courses`.</p>
            </div>
            <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-gray-300">overview</span>
          </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {(data?.courses ?? []).slice(0, 6).map((course) => (
              <div key={course.courseId} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-xs font-semibold text-gray-200">{course.progressPercent}%</span>
                </div>
            ...
            {!data?.courses?.length && (
              <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 p-4 text-sm text-gray-300 md:col-span-2">
        ...
        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)] sm:p-5">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-sm font-semibold text-white">Recomandări reale</h3>
            <span className="text-xs text-gray-300">bazate pe cont</span>
          </div>

          <div className="mt-4 space-y-2">
            {(data?.recommendations ?? []).map((item) => (
              <div key={`${item.kind}-${item.title}`} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                <div className="flex items-center justify-between gap-2">
                ...
                <p className="mt-2 text-[11px] uppercase tracking-[0.12em] text-gray-400">{item.reason}</p>
              </div>
            ))}
          ...
          <div className="mt-4 rounded-2xl border border-white/10 bg-gradient-to-r from-cyan-500/15 to-transparent p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-gray-300">Schema warnings</p>
      ...
      <div className="grid gap-3 xl:grid-cols-[1fr_1fr_0.9fr]">
        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)] sm:p-5">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-white">XP în ultimele 7 zile</h3>
            <span className="text-xs text-gray-300">din ledger</span>
          </div>

          <div className="mt-4 flex h-56 items-end gap-2 rounded-2xl border border-white/10 bg-white/5 p-4">
            {(data?.xpChart ?? []).map((item) => {
              const height = Math.max(10, Math.round((item.points / xpMax) * 100));
              return (
                <div key={item.date} className="flex flex-1 flex-col items-center gap-2">
                  <div className="flex h-40 w-full items-end rounded-xl bg-[#030712] p-1 shadow-sm">
                    <div className="w-full rounded-lg bg-cyan-400" style={{ height: `${height}%` }} />
                  </div>
                  <div className="text-[10px] uppercase tracking-[0.12em] text-gray-400">{item.label}</div>
                  <div className="text-xs font-semibold text-white">{item.points}</div>
                </div>
              );
            })}
          </div>
        ...
        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)] sm:p-5">
          ...
          <div className="mt-4 space-y-3">
            {(data?.activityFeed ?? []).map((item) => (
              <div key={`${item.kind}-${item.createdAt}-${item.title}`} className="rounded-2xl border border-white/10 bg-white/5 p-3">
        ...
        <article className="rounded-3xl border border-white/10 bg-[#030712] p-4 shadow-[0_8px_24px_rgba(15,23,42,0.03)] sm:p-5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-300">reale</span>
          </div>
          ...
          <div className="mt-4 space-y-2">
            {(data?.achievements ?? []).map((achievement) => (
              <div key={achievement.id} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                ...
                <p className="mt-2 text-[11px] uppercase tracking-[0.12em] text-gray-400">+{achievement.xpReward} XP</p>
              </div>
            ))}
