...
async function getResetRow(email: string) {
  ...
  const response = await fetch(
    `${supabaseUrl}/rest/v1/instructor_password_resets?select=*&email=eq.${encodeURIComponent(normalizeEmail(email))}&limit=1`,
...
export async function POST(request: Request) {
  try {
    ...
    if (expectedHash !== resetRow.code_hash) {
      await fetch(`${supabaseUrl}/rest/v1/instructor_password_resets?id=eq.${encodeURIComponent(resetRow.id)}`, {
        method: "PATCH",
        headers: {
          ...getSupabaseHeaders(),
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Failed to reset instructor password." },
      { status: 500 },
    );
  }
}